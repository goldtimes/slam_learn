...
class GridNN {
   public:
    ...
    /// 获取最近邻
    bool GetClosestPoint(const PointXYZI& pt, PointXYZI& closest_pt, size_t& idx);
...
bool GridNN<dim>::SetPointCloud(CloudPtr cloud) {
    ...
    std::for_each(index.begin(), index.end(), [&cloud, this](const size_t& idx) {
        ...
        auto key = Pos2Grid(lidar_utils::ToEigen<float, dim>(pt));
...
template <int dim>
bool GridNN<dim>::GetClosestPoint(const PointXYZI& pt, PointXYZI& closest_pt, size_t& idx) {
    // 在pt栅格周边寻找最近邻
    ...
    CloudPtr nearby_cloud(new PointCloudXYZI);
    ...
    size_t closest_point_idx = bfnn::bfnn_point(nearby_cloud, lidar_utils::ToVec3f(pt));
