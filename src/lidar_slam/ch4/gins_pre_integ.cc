...
void GinsPreInteg::SetOptions(Options options) {
    // 信息矩阵
...
void GinsPreInteg::AddGnss(const GNSS& gnss) {
    ...
    if (!first_gnss_received_) {
        ...
        // 处理首个gnss
        this_frame_->timestamped_ = gnss.unix_time_;
...
void GinsPreInteg::Optimize() {
    ...
    // 就是将各种顶点和边放到优化器中
    // 上一时刻的r,t
    VertexPose* v0_pose = new VertexPose();
    v0_pose->setId(0);
    v0_pose->setEstimate(last_frame_->GetSE3());
    optimizer.addVertex(v0_pose);
    // 上一时刻的v
    VertexVelocity* v0_vel = new VertexVelocity();
    v0_vel->setId(1);
    v0_vel->setEstimate(last_frame_->v_);
    // 上一时刻的bg,ba
    VertexGyroBias* v0_bg = new VertexGyroBias();
    v0_bg->setId(2);
    v0_bg->setEstimate(last_frame_->bg_);
    VertexAcceBias* v0_ba = new VertexAcceBias();
    v0_ba->setId(3);
