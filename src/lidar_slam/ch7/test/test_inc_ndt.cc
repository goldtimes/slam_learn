...
int main(int argc, char** argv) {
    ...
    inc_ndt_lo::IncrementalNDTLO::Options options;
    options.ndt3d_options_.nearby_type_ =
        FLAGS_use_ndt_nearby_6 ? ndt_inc::IncNdt3d::NearbyType::NEARBY6 : ndt_inc::IncNdt3d::NearbyType::CENTER;
    options.display_realtime_cloud_ = FLAGS_display_map;
    inc_ndt_lo::IncrementalNDTLO inc_ndt_lo(options);

    bag_io
        .AddAutoPointCloudHandle([&inc_ndt_lo](sensor_msgs::PointCloud2::Ptr msg) -> bool {
            timer::Timer::Evaluate(
                [&]() {
                    SE3 pose;

                    inc_ndt_lo.AddCloud(lidar_utils::VoxelCloud(lidar_utils::PointCloud2ToCloudPtr(msg), 0.1), pose);
                },
                "NDT registration");
            return true;
        })
    ...
    if (FLAGS_display_map) {
        // 把地图存下来
        inc_ndt_lo.SaveMap("./data/ch7/map.pcd");
    }
