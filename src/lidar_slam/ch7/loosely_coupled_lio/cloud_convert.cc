...
void CloudConvert::LoadFromYAML(const std::string &yaml) {
    ...
    time_scale_ = config["preprocess"]["time_scale"].as<double>();
    int lidar_type = config["preprocess"]["lidar_type"].as<int>();
    num_scans_ = config["preprocess"]["scan_line"].as<int>();
    point_filter_num_ = config["point_filter_num"].as<int>();
...
void CloudConvert::AviaHandler(const livox_ros_driver::CustomMsg::ConstPtr &msg) {
    ...
    cloud_out_.resize(point_size);
    cloud_full_.resize(point_size);

    std::vector<bool> is_valid_pt(point_size, false);
    std::vector<uint> index(point_size - 1);
    for (int i = 0; i < index.size(); ++i) {
        index[i] = i + 1;
    }
    std::for_each(std::execution::par_unseq, index.begin(), index.end(), [&](const uint &i) {
        if ((msg->points[i].line < num_scans_) && ((msg->points[i].tag & 0x30) == 0x10) &&
