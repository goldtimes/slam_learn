...
class IESKF {
   public:
    ...
    // 迭代卡尔曼滤波器中需要外部输入H矩阵
    // 这里定义一个回调函数，在更新阶段调用，获取ndt匹配残差
    using CustomObsFunc = std::function<void(const SE3& init_pose, Eigen::Matrix<S, 18, 18>& HT_Vinv_H,
