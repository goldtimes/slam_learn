...
class PCLMapViewer {
   public:
    ...
    PCLMapViewer(const float& leaf_size, bool use_pcl_vis = true)
        : leaf_size_(leaf_size), tmp_cloud_(new PointCloudXYZI), local_map_(new PointCloudXYZI) {
        if (use_pcl_vis) {
    ...
    void SetPoseAndCloud(const SE3& pose, CloudPtr cloud_world) {
        ...
        *local_map_ += *tmp_cloud_;
        ...
        if (viewer_ != nullptr) {
            ...
            pcl::visualization::PointCloudColorHandlerGenericField<PointXYZI> fieldColor(local_map_, "z");
            viewer_->addPointCloud<PointXYZI>(local_map_, fieldColor, "local_map");
    ...
    void SaveMap(std::string path) {
        if (local_map_->size() > 0) {
            lidar_utils::SaveCloudToFile(path, *local_map_);
